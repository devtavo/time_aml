...
class UsuarioController
{
	...
	function sesione($params){
		$sql = new MySql();

		$sql->getRow("update vsqvbmwm_certificado.usuario1 set ultimo_acceso=now() where usuario='".$params->usuario."' ");
		return $sql->getRow("insert into vsqvbmwm_certificado.log_usuario(usuario,ultimo_acceso,formulario) values('".$params->usuario."',now(),'".$params->formulario."') ");
		// return "insert into vsqvbmwm_nucleo.log_usuario(usuario,ultimo_acceso,formulario) values('".$params->usuario."',now(),'".$params->formulario."') ";
	}
	public function login($user,$pass){
		$sql = new MySql();

		return $sql->getRows("select * from vsqvbmwm_certificado.usuario1 where usuario='".$user."' and clave='".$pass."' ");
	}
	function findBeyId($params){
		$pg = new MySql();
	...
	function ultimoAcceso($user_id){
		$pg = new MySql();
		return $pg->getRow("update vsqvbmwm_certificado.usuario1 set ultimo_acceso = now() where usuario_id = " . $user_id);
	}
}